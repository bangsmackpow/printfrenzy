...
- **Dependency upgrade — blocked by the Pages builder**: both security bumps **break the Cloudflare Pages build** and had to be reverted. next→16.3.5 and next-auth→beta.32/`@auth/core`→0.41.3 each fail the deprecated `@cloudflare/next-on-pages@1.13.16` bundler (peer `next: ">=14.3.0 && <=15.5.2"`; bundled `@vercel/next@4.12.4` rejects Next 16.3 output; `@auth/core` 0.41.3 fails the edge bundle). Confirmed by controlled Pages builds: `next`→16.3.5 (8f049372/00afe73c ❌), `@auth/core`→0.41.3 on next 16.2.6 (preview 7f7aab57 ❌), vs code + pinned deps (a627f348 ✅ — my code alone builds green). **The app stays pinned at `next@16.2.6` + `next-auth@5.0.0-beta.31`.** (Also fixed the stale `--minify` flag in `pages:build`; note Pages ignores `wrangler.toml` and uses the project's stored build command + compatibility_date, which remains 2024-11-18.)
...
- **CI security gate fixed & made real**: the workflow previously died at the SARIF-upload steps (missing `semgrep.sarif`) *before* reaching the npm audit; those uploads now skip-if-absent and the scanners are non-blocking. The blocking check is `npm audit --omit=dev --json | node .github/audit-gate.mjs`, failing on any production CRITICAL not in `.github/audit-allowlist.json`. The 15 pinned advisories (11 `next@16.2.6` + 4 `@auth/core@0.41.2`/`next-auth@beta.31`) are allowlisted in `.github/audit-allowlist.json` with an explicit removal condition (migrate off next-on-pages). A full-tree audit stays non-blocking/informational.
...
- **Deferred (the real fix)**: clearing the 15 allowlisted criticals (11 Next framework — incl. *middleware/proxy bypass* + *unbounded Edge Server-Action payload* — and 4 Auth.js, incl. the *existence-check fail-open*) requires migrating off the deprecated `@cloudflare/next-on-pages` to **OpenNext or vinext on Workers**, then bumping to patched Next ≥16.3.3 + `@auth/core` ≥0.41.3. `middleware`→`proxy` and Edge-runtime deprecation warnings are accepted (edge is required for Pages). The next-on-pages Vercel build step can't run on native Windows (`spawn bash ENOENT`) — it builds only in Cloudflare Pages CI (Linux).
