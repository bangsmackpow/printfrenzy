...
- **Dependency security**: `next-auth` beta.31 → **beta.32** (`@auth/core` 0.41.3) clears the Auth.js *fail-open existence check* + *email homoglyph* criticals (CI verified green). Next was evaluated at **16.3.5** — it clears the framework criticals and `next build` passes locally, but it **cannot build on Cloudflare Pages**: `@cloudflare/next-on-pages@1.13.16` peer-caps `next` at `<=15.5.2` and its bundled `@vercel/next@4.12.4` fails on Next 16.3 output (Pages builds `8f049372`/`00afe73c` failed at ~90s). **Pinned at `next@16.2.6`** — the highest version this builder produces. `wrangler.toml` `compatibility_date` bumped (note: Pages uses the project's *stored* build settings, not `wrangler.toml`).
...
- **CI security gate fixed & made real**: the workflow previously died at the SARIF-upload steps (missing `semgrep.sarif`) *before* reaching the npm audit; those uploads now skip-if-absent and the scanners are non-blocking. The blocking check is `npm audit --omit=dev --json | node .github/audit-gate.mjs`, failing on any production CRITICAL not in `.github/audit-allowlist.json`. The 11 pinned `next@16.2.6` framework advisories are allowlisted with an explicit removal condition (migrate off next-on-pages). A full-tree audit stays non-blocking/informational.
...
- **Deferred (the real fix)**: clearing the 11 allowlisted Next criticals (incl. *middleware/proxy bypass* + *unbounded Edge Server-Action payload*, which are relevant to this app) requires migrating off the deprecated `@cloudflare/next-on-pages` to **OpenNext or vinext on Workers** and bumping to patched Next ≥16.3.3. `middleware`→`proxy` and Edge-runtime deprecation warnings are accepted (edge is required for Pages). The next-on-pages Vercel build step can't run on native Windows (`spawn bash ENOENT`) — it builds only in Cloudflare Pages CI (Linux).
